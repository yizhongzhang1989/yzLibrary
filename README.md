...
Document of yzLibrary can be generated by doxygen. Config yzLibrary using cmake, then yzLib_Doxy will be generated. Open this file using doxygen, the document of yzLibrary can be generated automatically. doc_entry.html contains the entry point of the document.

It is highly recommended to install Graphviz first, so that the inheritance relationship of classes can be visualized more clearly. If the path to dot.exe is C:/Program Files/Graphviz/bin, then Graphviz_BIN_PATH could be set automatically. Otherwise, please set its value in cmake. 
